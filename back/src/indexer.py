    ...
    def resolve_entity(self, label: str, name: str) -> str:
        ...
        if label == "Concept":
            ...
            if name_clean.lower() in aliases_map:
                ...
                logging.debug(f"resolve_entity '{name}' resolved from aliases_map in {dt:.6f}s")
        ...
        for eid, props in existing_nodes:
            if eid == slug:
                dt = time.perf_counter() - t0
                logging.debug(f"resolve_entity '{name}' exact slug match in {dt:.6f}s")
                return eid
        ...
        # 2. Embedding similarity check using vectorized numpy operation
        valid_candidates = []
        for eid, props in existing_nodes:
            ...
            if candidate_emb is not None and (isinstance(candidate_emb, list) or isinstance(candidate_emb, np.ndarray)):
                ...
                if query_norm > 0:
                    ...
                    if len(matching_indices) > 0:
                        eid = valid_candidates[matching_indices[0]][0]
                        dt = time.perf_counter() - t0
                        logging.debug(f"resolve_entity '{name}' resolved by vectorized embedding similarity in {dt:.6f}s")
                        return eid
        ...
        for eid, props in existing_nodes:
            ...
            if node_name:
                ...
                if ratio > 0.95:
                    dt = time.perf_counter() - t0
                    logging.debug(f"resolve_entity '{name}' resolved by string similarity fallback in {dt:.6f}s")
                    return eid
                    
        dt = time.perf_counter() - t0
        logging.debug(f"resolve_entity '{name}' fallback to slug in {dt:.6f}s")
        return slug
