...
Orchestrates three logical stages (subagents):
  1. data_prep_agent   — build per-query records with outcome-aware NaNs
  2. stats_agent       — bootstrap CIs, Wilcoxon, McNemar, Friedman
  3. reporting_agent   — markdown sections and optional plots
"""
...
def stats_agent(
    ...
    if records:
        # Re-use top-level analysis but inject pre-built records via synthetic data
...
def generate_statistical_plots(stats_analysis: dict[str, Any], output_dir: Path) -> list[str]:
    ...
    # P-value heatmap for pairwise semantic_accuracy
