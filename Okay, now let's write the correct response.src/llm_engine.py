...
def strip_thinking_tokens(text: str) -> str:
    """
    Strips thinking tokens (... and unclosed trailing ...)
    from the LLM output.
    """
    if not text:
        return text
    ...
    def _ensure_model_loaded(self):
        if self.model is None:
            ...
            with con.suppress_stderr(), con.suppress_stdout():
                try:
                    # Pass fix_mistral_regex=True to resolve tokenizer regex warning
                    # Falls back to default loading if mlx_lm doesn't support tokenizer_config
                    self.model, self.tokenizer = load(
                        self.model_path,
                        tokenizer_config={"fix_mistral_regex": True}
                    )
                except TypeError:
                    con.warning("mlx_lm version does not support tokenizer_config; loading with default settings.")
